   ...
   */
      ...
      */
		self.open = function (path, callback) {
			...
			var initProject = function initProject() {
				...
				self.trigger('projectChange:start');
        ...
        */
				self.project = new _modulesProject.Project(path, function (success) {
					...
					self.trigger('projectChange:end');
				});
			};

			var sccTest = path.split('.').pop();
			if (sccTest === 'scc') {
				var unzip = require('cross-unzip');
				var pathUtil = require("path");
				var mkdirp = require("mkdirp");
				self.ui._fileSave(function (directory) {
					mkdirp(directory, function (err) {
						console.log(path);
						unzip(path, directory, function (err) {
							if (err) {
      ...
      */
		self.update = function (callback) {
			...
			var pathUtil = require("path");
			var mkdirp = require("mkdirp");
			var thisVersion = jetpack.read('./package.json', 'json').version;
									...
									if (err) {
										...
										zipfile.on("close", function (err) {
											if (!err) {
												...
												self.set('recentProjects', appSettings.recentProjects);
												...
												if (self.environment === 'production') {
													...
													try {
														var newVersion = jetpack.read('./package.json', 'json').version;
														var infoPlistPath = self.path.split('/');
														infoPlistPath.pop();
														infoPlistPath.pop();
														infoPlistPath = infoPlistPath.join('/') + "/Info.plist";
														var info = jetpack.read(infoPlistPath);
														var matcher = new RegExp('<string>' + thisVersion + '</string>', 'g');
														info = info.replace(matcher, '<string>' + newVersion + '</string>');
														jetpack.write(infoPlistPath, info);
													} catch (err) {
														self.trigger('error', {
															message: "Could not update the version number. Application should function fine.",
															error: err
														}, 7);
													}
												}
