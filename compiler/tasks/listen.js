...
gulp.task('reload', ['recompile'], function() {
	io.emit('reload', utils.get('projectSettings'));
});

gulp.task('recompile', function() {
	runSequence('html', ['css', 'js'], 'html-meta');
});

var currentData;

gulp.task('reload-css', function() {
	currentData = utils.get('projectSettings').components;
	runSequence('css', 'html-meta', 'emit-reload');
});

gulp.task('emit-reload', ['recompile'], function() {
	var updatedData = utils.get('projectSettings').components;

	// Convert to strings, so we can compare two large objects
	if (JSON.stringify(updatedData) == JSON.stringify(currentData)) {
		io.emit('reload-css', utils.get('projectSettings'));
	} else {
		io.emit('reload', utils.get('projectSettings'));
	}
});
...
gulp.task('listen', ['build'], function () {
	...
	chokidar.watch(assetFiles).on('all', function(event, path) {
		var data = {
			event: event,
			path: path,
			project: utils.get('projectSettings')
		}
		io.emit('reload-asset', data);
	});
	// gwatch(assetFiles).pipe(
